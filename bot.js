...
var modules = {
        init: function()
        {
                ...
                // Zend Framework documentation beautifier
                if(location.href.matches('http://framework.zend.com/apidoc/*/db_*'))
                        modules.load('prettyzend');
               
                // Adds Ctrl+Enter support to phpmyadmin's SQL entry window
                if(location.href.matches('http://127.0.0.1:8089/phpmyadmin/*'))
                        modules.load('ctrl+enter support');
               
                // Hides the chatbox from blog.livenewschat.tv
                if(location.href.matches('http://blog.livenewschat.tv/rockinroosters/'))
                        modules.load('cleanup rockinroosters');
               
                // Makes mail.aol.com look a bit nicer
                if(location.href.matches('http://mail.aol.com/*/aol-*/en-us/Suite.aspx'))
                        modules.load('better aol', true);
               
                // Hverse bot
                if(location.href.matches('http://hentaiverse.org*'))
                        modules.load('IWBTH');
                       
                // Comcast TV-listing time scroller
                if(location.href.matches('http://xfinitytv.comcast.net/tv-listings*'))
                        modules.load('comcast/tv-listings');
        },
       
        _raw: {
                'prettyzend': function()
                {
                        var cont = unsafeWindow.frames['content'];
                        if(cont)
                        {
                                cont.getElementsByTagName('h1')[0].style.position = 'static';
                               
                                for(var i=0, j=cont.getElementsByTagName('div'); i<j.length; ++i)
                                {
                                        if(j[i].className == 'file-nav')
                                        {
                                                j[i].style.position = 'static';
                                                break;
                                        }
                                }
                        }
                ...
                'ctrl+enter support': function()
                {
                        var txtarea = document.getElementById('sqlquery');
                        if(txtarea)
                        {
                                var sqlGo, handler = function(e)
                                {
                                ...
                                {
                                        var item = j[i];
                                        if(item.type == 'submit' && item.name == 'SQL' && item.value == 'Go')
                                        {
                                                sqlGo = item;
                                                break;
                                        }
                                ...
                                txtarea.addEventListener('keydown', handler);
                                window.addEventListener('keydown', handler);
                        }
                },
               
                'cleanup rockinroosters': function()
                {
                        setTimeout((function()
                        {
                                document.getElementById('chat_embed').parentNode.removeChild(document.getElementById('chat_embed'));
                                document.getElementById('sharebar').parentNode.removeChild(document.getElementById('sharebar'));
                        }), 5000);
                },
                ...
                {
                        var times = 0,
                                handlr = setInterval(function()
                                {
                                        times++;
                                        if(times >= 30) clearInterval(handlr); // Will wait for 30 seconds for AOL to load
                                        if(document.getElementById('dojox_GridView_0'))
                                        {
                                                clearInterval(handlr);
                                                moo.load(function()
                                                {
                                                        var parent = null,
                                                                busy = false,
                                                                folderChanged = function()
                                                                {
                                                                        ...
                                                                        var unread = parseInt(parent.getElement('div.unseen').get('text') || 0).limit(0, 9999);
       
                                                                        if(unread && !parent.retrieve('blinker'))
                                                                        {
                                                                                parent.store('blinker', (function(){parent.highlight('#FFBCBC !important')}).periodical(500));
                                                                                document.body.grab(new Element('div#DG-warning', {
                                                                                        text: 'You have ' + unread + ' unread Professional message'+(unread == 1?'':'s')+'!',
                                                                                        styles: {
                                                                                                position: 'fixed',
                                                                                                top: 0, left: 0,
                                                                                                width: '100%',
                                                                        ...
                                                                        else if(!unread && parent.retrieve('blinker'))
                                                                        {
                                                                                clearInterval(parent.retrieve('blinker'));
                                                                                document.id('DG-warning').dispose();
                                                                                parent.store('blinker', false);
                                                                        }
                                                                       
                                                                        busy = false;
                                                                };
                                                       
                                                        $$('div[dojoattachpoint="myFoldersContainer"] .navItem .text span').every(function(item)
                                                        {
                                                                if(item.get('text') == 'Professional')
                                                                {
                                                                        parent = document.id(item.parentNode.parentNode);
                                                                        folderChanged();
                                                                        return false;
                                                                }
                                                        ...
                                                        if(!parent) return console.error('Error: could not find the "Professional" folder!');
                                                       
                                                        var previous = null;
                                                       
                                                        (function()
                                                        {
                                                                if(parent.getElement('div.unseen').get('text') != previous)
                                                                {
                                                                        if(!busy)
                                                                        {
                                                                                folderChanged();
                                                                                previous = parent.getElement('div.unseen').get('text');
                                                                        }
                                                                }
                                                        }).periodical(2000);
                                                });
                ...
                'comcast/tv-listings': function(){
                        setTimeout(function(){ location.reload(); }, (30-(new Date()).getMinutes()%30)*60*1000);
                },
